...
inline void KinectCV :: Reconnect(void){
  std :: string serial = kinect.getSerial();
  kinect.close();
  kinect.open(serial);
}
 ...
 * void setup(const std :: string &serial)                   *
 * ========================================================= */
inline void KinectCV :: setup (const std :: string &serial){
...
inline bool KinectCV :: update(void){
  ...
  if(kinect.isFrameNew()){
    ...
    for(int i = 0, size = contourfinder.blobs.size() ; i < size ; ++ i){
      ...
      m.addIntArg(pos.x);
      m.addIntArg(pos.y);
      m.addIntArg( pixels[int(Size().x * pos.y + pos.x)] );
...
inline ofxOscMessage& KinectCV :: osc(ofxOscMessage &m){
  ...
  if(m.getAddress() == "/kinect/binarydisplay"){
